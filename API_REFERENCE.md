...
- 統計利用の注意: 教材1件ごとに1 GET（HTML）なので全件収集はリクエスト数大。レート配慮＋キャッシュ必須。設問文・解答も含まれるが**メタデータ（passed/score/answered_at/total_score）のみ扱い、コンテンツ本文は保存しない**（ToS 第9条）。未受験教材でのレスポンスは要確認。
