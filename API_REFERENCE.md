...
### 【重要】必修以外（課外授業＝`service=advanced`）のデータモデル（2026-07-18・本家UIと突合して確定）
`GET /v3/dashboard/my_courses?service=advanced&limit=50&offset=0` で必修以外（選択科目・講座・大学連携。type:"advanced"）を取得。**必修(n_school)とは進捗モデルが別軸**で、本家「マイコース > 課外授業」タブの3表示に対応:
- **理解度%** = `comprehension`（教材の閲覧進捗）。`(good+perfect)/limit`。limit=閲覧対象の教材数（動画/ガイド/授業の合計）、good=閲覧済み。表示は **floor**（例: 英文読解ベーシック 2/133 → 本家表示「理解度1%」）。コース詳細/`?mode=batch` の `progress.comprehension` で取得。
- **習熟度テスト N/M** = `my_courses` の `progress.total_count`/`passed_count`（＝旧実装が「章数」と誤認した値の正体。英文読解 total_count:1）。
- **進捗度%** = 総合（現データでは理解度とほぼ一致。理解度で代用可）。
- **checkpoint** `{total, clear}` = 各教材のチェックポイント（補助軸）。
- 教材種別（章の `class_headers[]`＝section/lesson構造。resource_type: movie/guide/lesson等）。各教材に `progress.comprehension` と `checkpoint` が付く。
- **相互排他**: 必修(n_school)コースの progress は `total_materials`/`passed_materials`（教材数・例215/3）と `total_chapters`（章数12）が主。comprehension も付くが必修は total_materials で判定するのが本家流。advanced は total_materials が無く comprehension が主。
- **習熟度テスト(short_test)の個別結果**は未受験だとセッションが無く `/…/short_test_sessions/{id}/result` は404（受験後に取得可・OSS調査で `short_test_result{passed,score}` 確認済み）。集計 `course.short_test{total_short_test, total_passed_short_test}` はコース詳細に含まれる。
- 実装: `fetchElectiveCourses`（my_courses×batch の2GET）＝ `src/courseApi.ts`。学習数(`learning_amounts`)は必修/非必修を分離不可なので推移は全体共通。
...
- **essay_report の提出を実観測**（2026-07-18・ユーザー自身の提出をCiCで監視）: `POST /v1/n_school/courses/{c}/chapters/{ch}/essay_reports/{id}/answerings` → 201。**n_school 系パス**（material系ではない）。直後の章再取得で **`passed: true` が即時反映**（論述＝観点別評価でも提出時点で自動合格扱い。人間採点待ちで pending にはならない）。提出後は `/contents/.../essay_reports/{id}/result` に遷移（essay系の結果 params は空のまま＝点数データなし）。
...
- 統計利用の注意: 教材1件ごとに1 GET（HTML）なので全件収集はリクエスト数大。レート配慮＋キャッシュ必須。設問文・解答も含まれるが**メタデータ（passed/score/answered_at/total_score）のみ扱い、コンテンツ本文は保存しない**（ToS 第9条）。
- **【訂正・2026-07-18】essay系（essay_test/essay_report＝論述）にも結果データはある**。スキーマが別で `data-essay-{test|report}-params` = `{ result: { answered_at: epoch秒, score: number|null }, answerings, teacherComments?, total_score }`（first/latest でなく**単一**。score は人間採点後に数値化・提出直後は null。essay_report には teacherComments 配列あり）。旧記載「essay系は params が空」は evaluation系スキーマでパースした取りこぼしで誤り。essay_report は提出即 passed:true（実観測）なので、アンカー時刻としてフルに使える。
- essay_report の採点運用（2026-07-18 観測）: score と teacherComments は**提出から数日遅れの一括付与**（提出直後は score:null・comments はnull埋め配列。7/15以降提出分は7/18時点で未採点）。コメントは設問別＋総評の計5件・全面肯定・定型文の完全一致がレポート間で複数（文体分析はプロジェクトノート参照。自動生成の可能性が濃厚だが公式一次情報なし）。
