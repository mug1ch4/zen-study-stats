...
- **コース/章画面の残りサマリ**: 「このコース/この章の残り: 動画NN時間・N本 / 確認テストN / レポートN」をバナー注入。**取得は各コース/章で最大1回**（成功・失敗とも負のキャッシュ。MutationObserver から高頻度で呼ばれても再取得しない＝リクエストストーム防止）。章数の多いコースはバッチURLを分割取得。
