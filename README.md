...
- **控えめなトースト通知**（左下・数秒で自動消滅）: 全体%の節目達成・今日の必要最低限を達成・5:00の日付更新間近・**週明けの「先週のまとめ」**、を通知（永続dedupで繰り返さない）。
...
- **年度レポート完了予測（北極星）**: 教材消化ペースを基に、**モンテカルロ(1000回)で完了日の分布**を算出。「◯までに完了(85%)・締切に間に合う確率X%」。バーンダウン(P15〜85帯＋完了分布＋必要ライン＋**目標日ライン**)、EWMAペース、曜日/祝日考慮、目標日から必要ペース逆算、手法別の見立て。目標日を設定すると、その完了日への理想ペース線をグラフに追加描画（入力に追従・アニメーション付き）。さらに **「一日の教材数から逆算」**（義務ペース＝締切に間に合う最低ペース以上のみ設定可）で、指定した1日ペースでの完了見込み日を算出し、同じく理想ペース線を描画。締切は必修レポートの最終提出期限を動的取得。
