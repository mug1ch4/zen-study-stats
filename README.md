...
- **コンパクトな要点＋直下に詳細タブ**（`[推移][予測][教科]`）: 常時表示は要点ストリップ（累計・今日・平均・連続）だけに絞り、大きなグラフを含む詳細をトグルで隠さず**直下に3タブで常時表示**（この拡張の主役）。各タブは遅延描画。
  ...
  - **分析**: 先頭に **モチベーション「今日のひとこと」**（行動科学の実証手法に基づくナッジ＝下記）＋「あなたの学習傾向」— 必修アドバイス／**トレンド判定**（線形回帰の傾き＋Mann-Kendall検定で「伸びている/横ばい/落ちている」、週モメンタム、ラグ1自己相関で"好日の連鎖"）／**分布サマリー**（中央値＝普段の1日・上位1割＝好調日・0の日・パレート集中度）／曜日リズム（**Kruskal-Wallis検定で差が有意かノイズかを明示**＝小標本での過信防止）／時間帯（完了検知でその時刻を記録）／月ごと（前月比）／祝日／継続（CV＋学習日率＋最長空白＋バースト度）。統計関数は `src/stats.ts`（純関数）。
...
- **視聴任意教材の扱い**: 章内の `material_type: "supplement"`（視聴任意の補助動画）は、本家の進捗カウント（`total_count`/`passed_materials`）に含まれない（HAR＋実測で確認）。拡張の動画時間・残りサマリ・教科別シェアも同様に **main のみを集計**し、本家の必修進捗と母集団を揃えている。
