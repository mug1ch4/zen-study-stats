...
- **サイト全体ダークモード**（自作）: filter反転でなく動的リマップ（白面→暗色/暗文字→明色）。**低彩度の文字だけ再マップし、色付きの文字（ブランド青・成功緑・警告赤/橙）は意味を保持**。スキャンは読み書き二相＋シグネチャキャッシュで軽量。本家ナビに馴染む「テーマ」トグル、`chrome.storage`で永続化。
